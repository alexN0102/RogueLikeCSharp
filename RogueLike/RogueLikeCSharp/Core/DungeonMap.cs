...
namespace RogueSharpRLNetSamples.Core
{
    public class DungeonMap : Map
    {
        ...
        public IEnumerable<RogueSharp.Point> GetMonsterLocations()
        {
            return _monsters.Select(m => new RogueSharp.Point
            {
                X = m.X,
                Y = m.Y
            });
        ...
        public IEnumerable<RogueSharp.Point> GetMonsterLocationsInFieldOfView()
        {
            return _monsters.Where(monster => IsInFov(monster.X, monster.Y))
               .Select(m => new RogueSharp.Point { X = m.X, Y = m.Y });
        }
        ...
        public RogueSharp.Point GetRandomLocation()
        {
            int roomNumber = Game.Random.Next(0, Rooms.Count - 1);
            RogueSharp.Rectangle randomRoom = Rooms[roomNumber];
        ...
        public RogueSharp.Point GetRandomLocationInRoom(RogueSharp.Rectangle room)
        {
