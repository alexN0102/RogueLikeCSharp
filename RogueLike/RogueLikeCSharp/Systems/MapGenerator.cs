...
namespace RogueSharpRLNetSamples.Systems
{
    public class MapGenerator
    {
        ...
        private void CreateMap(RogueSharp.Rectangle room)
        {
            ...
            foreach (var room in _map.Rooms)
            {
                if (Dice.Roll("1D10") < 7)
                {
                    ...
                    for (int i = 0; i < numberOfMonsters; i++)
                    {
                        ...
                        {
                            RogueSharp.Point randomRoomLocation = _map.GetRandomLocationInRoom(room);
                ...
                if (Dice.Roll("1D10") < 3)
                {
                    if (_map.DoesRoomHaveWalkableSpace(room))
                    {
                        ...
                        if (randomRoomLocation != null)
                        {
                            ...
                            RogueSharp.Point location = _map.GetRandomLocationInRoom(room);
        ...
        private void PlaceAbility()
        {
            if (_level == 1 || _level % 3 == 0)
            {
                try
                {
                    ...
                    RogueSharp.Point location = _map.GetRandomLocationInRoom(_map.Rooms[roomIndex]);
